...
namespace librealsense
{
        ...
        _static_scene_captured(false)
    {
        ...
        hole_filling_mode->set_description(hf_fill_from_static, "Fill from static");
        hole_filling_mode->set_description(hf_python_sucks, "Python sucks");
    ...
    rs2::frame hole_filling_filter::process_frame(const rs2::frame_source& source, const rs2::frame& f)
    {
        ...
        if (_static_scene_captured)
            ...
            if (_extension_type == RS2_EXTENSION_DISPARITY_FRAME)
                apply_hole_filling_first<float>(const_cast<void*>(tgt.get_data()));
            else
                apply_hole_filling_first<uint16_t>(const_cast<void*>(tgt.get_data()));
