...
namespace librealsense
{
    ...
    protected:
        ...
        void apply_hole_filling(void * image_data)
        {
            ...
            case hf_nearest_from_around:
                ...
                break;
            case hf_fill_from_static:
                holes_fill_static(data, _width, _height, _stride);
                break;
            case hf_python_sucks:
                holes_fill_petan(data, _width, _height, _stride);
                break;
            default:
        ...
        template<typename T>
        void apply_hole_filling_first(void* image_data)
        {
            bool fp = (std::is_floating_point<T>::value);
            T* data = reinterpret_cast<T*>(image_data);
            ...
            _static_scene_data = new uint16_t[_width*_height]; // neberu v úvahu stride
            memcpy(_static_scene_data,data, _width * _height* _bpp);
            _static_scene_captured = true;
        }
        // Implementations of the hole-filling methods
        template<typename T>
        inline void holes_fill_static(T* image_data, size_t width, size_t height, size_t stride)
        {
            ...
            T* p = image_data;
            uint16_t* p2 = _static_scene_data;
        ...
        // Implementations of the hole-filling methods
        template<typename T>
        inline void holes_fill_petan(T* image_data, size_t width, size_t height, size_t stride)
        {
            std::function<bool(T*)> fp_oper = [](T* ptr) { return !*((int*)ptr); };
            std::function<bool(T*)> uint_oper = [](T* ptr) { return !(*ptr); };
            auto empty = (std::is_floating_point<T>::value) ? fp_oper : uint_oper;
    ...
    private:
        ...
        bool                    _static_scene_captured;
        uint16_t*                  _static_scene_data;
    };
